// ============================================================
// กุยช่ายสวรรค์ — Edge Function "secure-api"
...
// ============================================================
// ===== ผู้ช่วยร้าน (askAI) — Gemini key อยู่ฝั่ง server, context ลับดึงจาก Supabase =====
...
// ===== whitelist ตาราง/คอลัมน์ สำหรับเครื่องมือดึงข้อมูลของผู้ช่วยร้าน =====
...
async function execStockWrite(body: any) {
  ...
  const today = todayStr();
  const now = new Date().toISOString();
  const noteFinal = note || "บันทึกผ่านผู้ช่วยร้าน";
...
const MARU_TOOLS = [{
  functionDeclarations: [{
    ...
    description: "ดึงข้อมูลแถวจริงจากฐานข้อมูลร้านกุยช่ายสวรรค์ เพื่อตอบคำถามรายละเอียด เช่น รายการเบิก/รับเข้าล่าสุดของสินค้า, ยอดขายแยกช่องทาง (grab/lineman/cash...) รายวัน, ค่าใช้จ่ายแต่ละรายการ, การเข้างาน ฯลฯ ใช้เมื่อบริบทที่ให้มายังไม่มีตัวเลข/แถวที่ถาม แล้วรวม/นับ/หาค่าจากแถวที่ได้เอง ห้ามเดาตัวเลข",
    parameters: {
      type: "OBJECT",
      properties: {
...
async function askAI(body: any) {
  ...
  const today = todayStr();
  const system = [
    'คุณคือ "ผู้ช่วยร้าน" ผู้ช่วย AI ประจำร้านกุยช่ายสวรรค์ (ร้านกุยช่ายในไทย ย่านพันทิพย์ งามวงศ์วาน)',
...
async function genPromoCaption(body: any) {
  ...
  const sys = [
    "คุณคือทีมการตลาดของร้านกุยช่ายสวรรค์ (ร้านกุยช่ายในไทย ย่านพันทิพย์ งามวงศ์วาน)",
...
async function genPromoImage(body: any) {
  ...
  let instr = imgs.length > 1
    ? "นี่คือรูปสินค้าจริงหลายรูปของร้านกุยช่ายสวรรค์ ช่วยนำสินค้าจากทุกรูปมารวม/จัดวางไว้ในภาพโปรโมชั่นเดียวกันที่ดูน่ากิน จัดแสง พื้นหลัง และองค์ประกอบให้สวยแบบมืออาชีพ คงตัวสินค้าแต่ละชิ้นให้เหมือนเดิม ไม่เปลี่ยนชนิดอาหาร โทนภาพสว่างสดใส"
    : "นี่คือรูปสินค้าจริงของร้านกุยช่ายสวรรค์ ช่วยปรับให้เป็นภาพโปรโมชั่นที่ดูน่ากินขึ้น จัดแสง พื้นหลัง และองค์ประกอบให้สวยแบบมืออาชีพ โดยคงตัวสินค้า (กุยช่าย/ของในรูป) ให้เหมือนเดิม ไม่เปลี่ยนชนิดอาหาร โทนภาพสว่างสดใส";
